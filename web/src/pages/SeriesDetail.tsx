...
export default function SeriesDetailPage() {
      ...
      onQuickAddToPlaylist: async (mediaIds, playlistId) => {
        try {
          ...
          const name =
            ...
            "播放列表";
          message.success(`已添加 ${mediaIds.length} 项到「${name}」`);
  ...
  }, [allMediaIds, representativeMedia?.scraped, detail?.library_id, recentPlaylistMenu, reloadSeries]);
            ...
            <div className={md.introMain}>
              ...
              {scoreText ? (
                <div className={md.scoreRow}>
                  <span className={md.criticScore}>评分 {scoreText}</span>
                </div>
              ) : null}
            ...
            <div className={md.castRow}>
              {castList.map((member, idx) => (
                <div key={`${member.name}-${idx}`} className={md.castCard}>
                  ...
                  <div className={md.castRole}>{member.role || "演员"}</div>
                </div>
              ))}
            </div>
          </div>
