...
func (h *Handler) ListFavorites(c *gin.Context) {
	...
	for rows.Next() {
		...
		var fileID, title, orig, path, ftype, format, status, created sql.NullString
		var dur, w, h, br sql.NullInt64
		if err := rows.Scan(&mid, &libID, &fileID, &title, &orig, &path, &ftype, &dur, &w, &h, &br, &format, &status, &created); err != nil {
			continue
		}
