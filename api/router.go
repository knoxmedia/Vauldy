...
func NewEngine(cfg *config.Config, application *app.App, deps handler.Dependencies) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		play.Use(middleware.RequireAuthentication(cfg, true))
		{
			...
			// DRM license/packaging routes are mounted by the commercial DRM
			// module (coreiface.DRMModule.RegisterRoutes). The community build
			// leaves the handle nil, so /drm/* behaves as absent (404).
			if deps.Instant != nil {
		...
		adm.Use(middleware.RequireAdmin())
		{
			...
			adm.POST("/subtitle/task/:mediaId/cancel", h.CancelSubtitleTask)
			adm.POST("/subtitle/task/:mediaId/run-now", h.RunNowSubtitleTask)
			adm.DELETE("/subtitle/task/:mediaId", h.DeleteSubtitleTask)
			adm.POST("/subtitle/task/batch", h.BatchSubtitleTasks)
			...
			adm.POST("/lyric/task/batch", h.BatchLyricTasks)
			...
			adm.POST("/atrack/task/batch", h.BatchAtrackTasks)
			adm.POST("/media/:id/keyframe", h.EnqueueKeyframeExtraction)
			adm.POST("/media/:id/encrypt-assets", h.EncryptMediaAssets)
			adm.GET("/encrypt/task", h.ListEncryptTasks)
			adm.POST("/encrypt/task/:id/cancel", h.CancelEncryptTask)
			adm.POST("/encrypt/task/:id/reset", h.ResetEncryptTask)
			adm.DELETE("/encrypt/task/:id", h.DeleteEncryptTask)
			adm.POST("/encrypt/task/batch", h.BatchEncryptTasks)
			adm.GET("/keyframe/task", h.ListKeyframeTasks)
			adm.POST("/keyframe/task/:mediaId/retry", h.RetryKeyframeTask)
			adm.POST("/keyframe/task/batch", h.BatchKeyframeTasks)
			...
			// --- DRM (commercial-only) ---
			// Mounted by the DRM module so the community build compiles without
			// the DRM handler and answers 404 for /drm/* and /admin/drm-*.
			if drmMod := coreiface.DRMModuleHandle(); drmMod != nil {
				drmPlay := play.Group("/drm")
				drmMod.RegisterRoutes(drmPlay, adm)
			}
		}
	}
