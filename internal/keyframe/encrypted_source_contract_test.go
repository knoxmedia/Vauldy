...
func TestEncryptedSourceStreamDecryptAfterPlaintextRemoval(t *testing.T) {
	...
	defer db.Close()
	vault, err := keystore.NewVault("keyframe-enc-key", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	dir := t.TempDir()
	plain := []byte("ftypmp42" + string(bytes.Repeat([]byte{0x33}, 2048)))
	...
	if ff.Stdin == nil {
		t.Fatal("keyframe stream_decrypt requires pipe after plaintext removal")
	}
}
