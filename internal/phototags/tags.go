...
// NormalizeMetaJSON normalizes all persisted photo tag arrays while preserving unknown fields.
func NormalizeMetaJSON(raw string) string {
	var root map[string]any
	if strings.TrimSpace(raw) == "" || json.Unmarshal([]byte(raw), &root) != nil {
		return raw
	}
	photo, ok := root["photo"].(map[string]any)
	if !ok {
		return raw
	}
	changed := false
	for _, key := range []string{"tags", "ai_tags", "manual_tags"} {
		values, ok := photo[key].([]any)
		if !ok {
			continue
		}
		before := make([]string, 0, len(values))
		valid := true
		...
		after := NormalizeTags(before)
		if !tagSlicesEqual(before, after) {
			photo[key] = after
			changed = true
		}
	}
...
func tagSlicesEqual(a, b []string) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
